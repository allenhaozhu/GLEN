...
class Net(torch.nn.Module):
    def __init__(self, args):
        ...
        self.conv1 = torch.nn.Linear(args.num_features, args.nhid)
        self.conv2 = torch.nn.Linear(args.nhid, args.output)
        self.do1 = torch.nn.Dropout(p=0.5)
        self.do2 = torch.nn.Dropout(p=0.5)
        self.bn = torch.nn.BatchNorm1d(args.output, affine=False)
        # self.conv1 = SNLinear(args.num_features, args.nhid)
        # self.conv2 = SNLinear(args.nhid, args.output)

    def forward(self, F1):

        z = torch.relu(self.conv1(self.do1(F1)))
        z = self.conv2(self.do2(z))
        #z = self.bn(z)
        #z = (z - z.mean(0))
...
class Single(torch.nn.Module):
    def __init__(self, args):
        ...
        #geotorch.orthogonal(self.conv1, "weight")
        self.do1 = torch.nn.Dropout(p=0.5)
        # self.conv1 = SNLinear(args.num_features, args.nhid)
        # self.conv2 = SNLinear(args.nhid, args.output)
